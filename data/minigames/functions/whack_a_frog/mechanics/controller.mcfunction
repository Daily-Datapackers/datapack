...
execute if score @s timer1 matches 0 if score @s timer2 matches 1.. run scoreboard players remove @s timer2 1
...
execute if score @s timer1 matches 0 run function main:codelib/rng

# Activates the frog with the previously selected frog id
execute if score @s timer1 matches 0 as @e[type=frog,tag=whack_a_frog,distance=..2,scores={timer1=0}] if score @s whack_a_frog.id = rng temp run function minigames:whack_a_frog/mechanics/set_frogtimer

# Resets the sub timer of the controller
execute if score @s timer1 matches 0 if score @s timer2 matches 1.. run scoreboard players set @s timer1 20
